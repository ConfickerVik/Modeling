...
def mmninf(**kargs):
    ...
    return ["M/M/n/inf", "inf", p_och, q, n_, k_n, t_c]
...
def mmnm(**kargs):
    ...
    p0 = (1 + \
    sum([(kargs["alph"]**k)/fac(k) for k in range(1, kargs["n"] + 1)]) + \
    ((kargs["alph"]**kargs["n"])/fac(kargs["n"])) * sum([(kargs["alph"]/kargs["n"])**s for s in range(1, kargs["m"]+1)])
    )**(-1)
    print(f"\tВероятность того, что в системе находится заявка: {p0}")
    pk = [((kargs["alph"]**k)/fac(k))*p0 for k in range(1, kargs["n"]+1)]
    print(f"\tВероятности того, что в системе находится k заявок: {pk}")
...
def print_table(lst_data):
    cols = {"1":14,
            ...
            "6":14,
            "7":14}
    head = ["Случай", "m", "Pотк", "Q", "n_", "kn", "tc"]

    print(f"+","-"*cols["1"],"+","-"*cols["2"],"+","-"*cols["3"],"+","-"*cols["4"],"+","-"*cols["5"],"+","-"*cols["6"],"+","-"*cols["7"],"+",sep="")
